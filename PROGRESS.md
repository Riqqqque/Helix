...
slice are real and testable, but Phase 0 is **BLOCKED** from completion because
the target Ubuntu install, service, permission, recovery-fault, and
reference-performance matrices have not run. Helix is not ready for production,
public network exposure, or a packaged public release.
...
| Rust workspace | TESTED | Nine founding crates compile on Windows with locked dependencies, formatting, Clippy warnings-as-errors, tests, and a release build. The declared Rust 1.88 MSRV and Linux targets still require hosted or clean-host validation. |
| Configuration | TESTED | Strict typed TOML, explicit overrides, loopback-only policy, absolute roots, and canonical disjoint state/static roots have adversarial coverage. Linux ownership and replacement races remain target-host gates. |
| Critical state | TESTED | SQLite WAL/FULL policy, schema versions 1–4, stable installation/node identity, integrity and semantic checks, unclean marker, online snapshots, process lease, private-file checks, and read-only CLI paths have portable tests. Migration retries reuse only a fully verified source/version/target/SHA-bound rollback snapshot; changed sources get a new snapshot, altered aliases fail closed, and at most 16 legacy partials are reconciled per open. Unix mode/owner/symlink and fault matrices remain blocked on Linux. |
...
| Daemon lifecycle | TESTED | Loopback listener, structured logging, API/static composition, exclusive writer lease, recovery-before-cleanup ordering, tracked blocking-task drain, and clean-marker ordering have focused tests. A live Windows run detected a forced unclean stop, passed the full state check, then handled a targeted Ctrl+C, logged a clean stop, and exited 0. Direct systemd behavior remains unvalidated. |
...
| HTTP foundation | TESTED | Versioned API, JSON API fallback, SPA fallback, restrictive headers, request IDs, compression, body/timeout limits, strict loopback Host validation, global concurrency bounds, and single-flight host sampling have service tests. Network-socket flood and Linux tests remain. |
| Host overview | TESTED | Real hostname, OS, architecture, kernel, uptime, CPU, memory, swap, storage, and cumulative network counters are collected on demand. Windows live data rendered two mounts and two interfaces; an unlabeled volume is represented as an explicit text omission rather than invalid empty text. Linux source validation remains. |
| Frontend | TESTED | The compiled Preact/Vite UI uses real protected APIs, honest degraded states, visibility-aware polling, semantic native meters, keyboard focus, reduced-motion support, responsive layouts, and System/Midnight/OLED/Light themes. Live Windows browser checks passed login, reload-to-login CSRF behavior, sign-out, desktop OLED, and a 320-pixel layout with no horizontal overflow or console errors. Formal screen-reader and representative-device review remain. |
| CI and dependency policy | IMPLEMENTED — UNVALIDATED | Actions and tool versions are pinned. Local Rust advisory/license/source, npm advisory, and secret scans pass. No hosted CI run has validated Ubuntu 24.04, Rust 1.88, or the checked-in workflow. |
| Packaging and installer | IMPLEMENTED — UNVALIDATED | Hardened systemd/sysusers/tmpfiles fixtures, checksum-producing Linux bundles, transactional package-file install/rollback, and conservative data-preserving uninstall exist. PowerShell and Bash parsers pass, but no Ubuntu install/upgrade/rollback/uninstall evidence exists. Artifacts are unsigned and must not be attached to a release. |
...
| Browser login/session/CSRF | TESTED | The loopback setup, owner claim, login, protected reads, CSRF compare-and-swap rotation, logout/revocation, Host/Origin/Fetch-Metadata checks, cookie flags, generic failures, and reload-to-login rule passed unit, API, and live compiled-asset flows. TLS, trusted proxies, MFA, Linux packaging, brute-force field testing, and independent review remain gates. |
...
1.94.1, Node 24.12.0, and npm 11.6.2 on Windows 11 x64.
...
The settled suite contains 152 Rust tests and 44 frontend tests. The final fresh
release-binary flow passed setup, owner claim, login, protected reads, CSRF
...
Release artifacts measured 7,568,384 bytes for `helixd.exe`, 2,857,472 bytes
for `helixctl.exe`, and 10,425,856 bytes combined. The compiled frontend
measured 91,465 bytes raw, 24,733 bytes gzip, and 21,838 bytes Brotli. These
Windows values are non-reference.

The Linux bundle scripts passed static parser checks. Pinned `cargo-audit`
0.22.2 scanned 197 locked dependencies against 1,226 loaded RustSec advisories
with no finding. Pinned `cargo-deny` 0.20.2 passed advisories, bans, licenses, and
sources with seven reviewed duplicate-version warnings. npm reported zero
vulnerabilities. Pinned Gitleaks 8.30.1 scanned about 1.19 MB of the current tree
and the pre-commit empty history with no leak. Hosted CI and target-host behavior
remain unverified.

This validation snapshot came from an uncommitted tree with no remote, so it
does not establish release provenance. Publishing the source does not
retroactively change that evidence; release gates must run from a committed,
reviewed revision.

## Packaged release gate

**BLOCKED.** Required Ubuntu installation, packaged owner creation, independent
authentication review, production master-key lifecycle, audit export and tamper
evidence, permissions/filesystem tests, restore, corruption/disk-full/power-loss
drills, signed update integrity, formal screen-reader and OS forced-colors review,
representative mobile review, game lifecycle matrices, and reference performance
have not passed. No signed binary/package release or deployment recommendation
is authorized by the current evidence.
