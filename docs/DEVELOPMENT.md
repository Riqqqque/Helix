...
On 64-bit systemd Linux, `./scripts/install-from-source.sh` compiles
this checkout and installs `helixd` on Debian/Ubuntu, Fedora/RHEL-family,
openSUSE, Arch, and other GNU-userland systemd distros. That is the local
package path, not `helix-privd`. See [Installation](INSTALLATION.md) and
