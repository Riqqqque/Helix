...
- A password used as the only factor must contain at least 15 Unicode code points. Helix accepts at least 64 and initially caps input at 256 code points and 1024 UTF-8 bytes to bound work. It imposes no character-class rule or periodic rotation. The current production path rejects a small built-in set of obvious defaults. The authentication crate has a pluggable compromised-password checker, but a maintained blocklist source and its offline/update/failure policy remain a public-release gate.
...
- A browser session uses a separate 32-byte random token. Only its domain-separated hash is stored. Sessions have server-enforced idle and absolute deadlines and capture the user's `auth_version`; password, status, and authorization changes invalidate prior versions.
...
- Every protected route requires both the session cookie and the session-bound synchronizer token in `X-Helix-CSRF`, including protected reads. CSRF tokens are random, stored only as hashes, and held only in frontend memory. Mutations additionally require an exact allowed `Origin` and same-origin Fetch Metadata when supplied.
- The rotation route also requires the current synchronizer token. A page reload therefore returns to login instead of using the cookie alone to mint a new proof.
...
- This stronger local rule is intentional: cookies are scoped by host, not TCP port, so another service on `localhost` can receive `helix_session`. It still cannot use that cookie against Helix without the current in-memory CSRF proof. TLS deployment must revisit cookie names, `Secure`, trusted proxy behavior, and origin policy as one reviewed boundary.
...
Requiring login after a reload is a deliberate usability cost in the loopback-HTTP phase. Helix will not weaken the second proof merely to provide silent cookie restoration.
...
- a loopback cookie without its in-memory proof cannot read protected data or rotate a replacement proof;
