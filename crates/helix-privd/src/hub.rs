...
mod tests {
    ...
    fn target(auth: MachineAuthKind) -> MachineTarget {
        MachineTarget {
    ...
    #[test]
    fn key_auth_argv_uses_hub_identity_and_dedicated_known_hosts() {
        let argv = hub().ssh_argv(&target(MachineAuthKind::Key), false);
        ...
        assert!(
            argv.iter()
                .any(|arg| arg == "UserKnownHostsFile=/var/lib/helix-hub/known_hosts")
        );
        let identity = argv
            .iter()
            .position(|arg| arg == "-i")
        ...
        assert_eq!(
            identity.map(String::as_str),
            Some("/var/lib/helix-hub/id_ed25519")
        );
        assert_eq!(
            argv.last().map(String::as_str),
            Some("operator@192.0.2.10")
        );
    ...
    #[test]
    fn batch_probe_argv_disables_prompts_and_forces_no_tty() {
        let argv = hub().ssh_argv(&target(MachineAuthKind::Password), true);
        assert!(argv.iter().any(|arg| arg == "BatchMode=yes"));
        assert!(!argv.iter().any(|arg| arg == "-tt"));
        assert!(argv.iter().any(|arg| arg == "PubkeyAuthentication=no"));
        assert!(!argv.iter().any(|arg| arg == "-i"));
    }

    #[test]
    fn system_auth_leaves_identity_unspecified() {
        let argv = hub().ssh_argv(&target(MachineAuthKind::System), true);
        assert!(!argv.iter().any(|arg| arg == "-i"));
        assert!(!argv.iter().any(|arg| arg == "IdentitiesOnly=yes"));
        assert!(!argv.iter().any(|arg| arg == "PubkeyAuthentication=no"));
    }

    #[test]
    fn parse_probe_reads_the_keyed_snapshot() {
        let probe = parse_probe(
            "junk\n__HX__\nH=builder\nK=6.8.0-79-generic\nN=16\nO=Ubuntu 24.04.3 LTS\nU=1234567\nL=0.42 0.50 0.61\nMT=33554432\nMA=16777216\nD=1000000000000 600000000000\nC=4\nF=2\n",
            42,
        );
        assert_eq!(probe.status, "online");
        assert_eq!(probe.hostname.as_deref(), Some("builder"));
        assert_eq!(probe.kernel.as_deref(), Some("6.8.0-79-generic"));
        assert_eq!(probe.os.as_deref(), Some("Ubuntu 24.04.3 LTS"));
        assert_eq!(probe.cpu_count, Some(16));
        assert_eq!(probe.uptime_seconds, Some(1_234_567));
        assert_eq!(probe.load.map(|load| load[0]), Some(0.42));
        assert_eq!(probe.mem_total_bytes, Some(33_554_432));
        assert_eq!(probe.disk_total_bytes, Some(1_000_000_000_000));
        assert_eq!(probe.containers_running, Some(4));
        assert_eq!(probe.failed_units, Some(2));
        assert_eq!(probe.latency_ms, Some(42));
    }

    #[test]
    fn parse_probe_rejects_output_without_the_marker() {
        let probe = parse_probe("H=builder\n", 7);
        assert_eq!(probe.status, "error");
        assert!(probe.hostname.is_none());
    }

    #[test]
    fn mac_parsing_accepts_colons_and_dashes_only() {
        assert!(parse_mac("3c:52:82:ab:12:34").is_ok());
        assert!(parse_mac("3C-52-82-AB-12-34").is_ok());
        assert!(parse_mac("3c5282ab1234").is_ok());
        assert!(parse_mac("3c:52:82:ab:12").is_err());
        assert!(parse_mac("gg:52:82:ab:12:34").is_err());
        assert!(parse_mac("").is_err());
    }

    #[test]
    fn host_validation_blocks_flags_and_shell_chars() {
        assert!(validate_host("192.0.2.10").is_ok());
        assert!(validate_host("builder.lan").is_ok());
    ...
    #[test]
    fn username_validation_blocks_flags_and_paths() {
        assert!(validate_username("operator").is_ok());
        assert!(validate_username("svc.backup-01").is_ok());
        assert!(validate_username("-lroot").is_err());
        assert!(validate_username("root/../x").is_err());
        assert!(validate_username("").is_err());
    }

    #[test]
    fn ssh_failures_classify_into_card_statuses() {
        assert_eq!(
        ...
        assert_eq!(
            classify_ssh_failure("ssh: connect to host x port 22: Connection refused").0,
            "unreachable"
        );
        assert_eq!(classify_ssh_failure("weird output").0, "error");
    }
}
